...
function scoreBuyerForSeller(buyer, seller) {
  ...
  const budgetScore =
    buyer.budgetMax == null
      ...
      : buyer.budgetMax >= seller.price
        ? 100
        : buyer.budgetMax + TOLERANCE >= seller.price
          ? Math.round(100 * (1 - (seller.price - buyer.budgetMax) / TOLERANCE))
          : 0;
  ...
  if (buyer.piecesMin != null && seller.pieces >= buyer.piecesMin) {
    ...
    if (extra === 1) tech += 10;
    if (extra >= 3) tech -= 5;
  } else if (buyer.piecesMin == null) {
    tech += 45;
  }

  if (buyer.surfaceMin != null && seller.surface >= buyer.surfaceMin) {
    tech += 45;
  } else if (buyer.surfaceMin == null) {
    tech += 45;
  }
...
export function matchSellerToBuyers(sellerProfile, topN = 5) {
  ...
  const scored = geoPool.map((buyer) => {
    ...
    const budgetMatch =
      buyer.budgetMax == null
        ? "none"
        : buyer.budgetMax >= sellerProfile.price
          ? "perfect"
...
export function getStatsMatches(buyerProfile, limit = 30) {
  ...
  // Dans getStatsMatches, dans le pool.map()
  const pool = SELLERS.filter((s) => s.role === "seller")
    .map((seller) => {
      const coords = getCoords(seller.ville); // 🔥 déjà là
      const distanceToBuyer =
        coords && buyerCoords
          ? distanceKm(seller.ville, buyerProfile.ville)
          : Infinity;
      ...
      return {
        ...seller,
        distanceToBuyer,
        lat: coords?.lat ?? null, // 🔥 AJOUT
        lng: coords?.lng ?? null, // 🔥 AJOUT
        buyerLat: buyerCoords?.lat ?? null, // 🔥 AJOUT
        buyerLng: buyerCoords?.lng ?? null, // 🔥 AJOUT
        villeScoreVal: scoreVille(
          seller.ville,
          buyerProfile.ville,
          30,
