...
/**
 * Construit l'objet normalized final avant upsert et matching.
 */
...
/* ═══════════════════════════════════════════════════════════════════════════
   computeNextSellerPopup
   Reçoit les critères APRÈS merge complet.
   Retourne le pop-up à déclencher MAINTENANT, ou null.
═══════════════════════════════════════════════════════════════════════════ */
function computeNextSellerPopup(sc) {
  // 1. PROXIMITE — ville connue, commodités pas encore array
  if (sc.ville && !Array.isArray(sc.proximite)) return "proximite";

  // 2. ETAT DU BIEN — tous critères texte remplis, etatBien absent
  const textReady =
    sc.ville &&
    sc.type &&
  ...
  if (textReady && (!sc.etatBien || !sc.etatBien.trim())) return "etatBien";

  // 3. NIVEAU ENERGETIQUE — etatBien ok, DPE absent
  if (
    sc.etatBien?.trim() &&
    (!sc.niveauEnergetique || !sc.niveauEnergetique.trim())
  )
    return "niveauEnergetique";

  // 4. IMAGES — DPE ok, images absentes
  if (
    sc.etatBien?.trim() &&
...
/* ═══════════════════════════════════════════════════════════════════════════
   CHAT ROUTE
   Flux corrigé :
   1. Injections pop-up (req.body)
   2. Premier appel IA SANS nextPopup (collecte/extraction pure)
   3. Merge des critères extraits
   4. Calcul nextPopup SUR L'ÉTAT MERGÉ
   5. Si nextPopup détecté → deuxième appel IA EN MODE A (annonce)
      et remplacement du reply
   6. Envoi réponse client avec trigger approprié
═══════════════════════════════════════════════════════════════════════════ */
app.post("/chat", authenticateToken, userQueueMiddleware, async (req, res) => {
  try {
    ...
    // ── 1. Injections directes depuis les pop-ups ──────────────────────────
    ...
    if (req.body.skipImages === true) session.criteria.imagesbien = [];

    const isInternalMessage = message.startsWith("__");

    // ── 2. Premier appel IA — extraction pure, SANS nextPopup ─────────────
    // L'IA collecte les critères du message utilisateur normalement.
    // On ne lui dit pas encore quel pop-up va s'ouvrir car on ne le sait pas
    // encore (on ne connaît pas les critères après merge).
    let aiResponse = { message: "", criteria: { ...session.criteria } };

    if (!isInternalMessage) {
      // Message utilisateur réel → extraction
      try {
        aiResponse = await aiChatWithCriteria(message, session.criteria, {
          phase: session.phase,
          matchingProfiles: session.matches,
          role: userRole,
          nextPopup: null, // pas encore calculé
        });
        console.log(
          "🤖 [AI STEP1 - EXTRACT]",
        ...
        console.error("[CHAT] Erreur AI step1:", e);
        aiResponse = {
    ...
    let nextPopup = null;
    if (userRole === "seller") {
      nextPopup = computeNextSellerPopup(sc);
    }
    console.log("🎯 NEXT POPUP:", nextPopup);

    // ── 5. Si pop-up détecté → deuxième appel IA en MODE A ───────────────
    // Ce deuxième appel remplace le reply par le message d'annonce du pop-up.
    // L'IA sait maintenant exactement quoi dire.
    let reply = aiResponse.message || "";

    if (nextPopup && userRole === "seller") {
      // ── Si message interne (post pop-up) : texte statique DIRECT, zéro appel LLM ──
      // aiParsee MODE A ne fait plus d'appel réseau → réponse immédiate < 1ms
      try {
        const announceResponse = await aiChatWithCriteria(
          `ANNOUNCE_${nextPopup.toUpperCase()}`,
          sc,
          {
            phase: session.phase,
            matchingProfiles: session.matches,
            role: userRole,
            nextPopup,
            isAnnounceCall: true, // ← déclenche MODE A statique dans aiParsee
          },
        );
        console.log("⚡ [ANNOUNCE STATIQUE]", announceResponse.message);
        if (announceResponse.message?.trim()) {
          reply = announceResponse.message;
        }
      } catch (e) {
        console.error("[CHAT] Erreur announce:", e);
        const FALLBACK = {
          proximite: `${sc.ville} enregistrée. Carte des commodités en cours d'affichage.`,
          etatBien: "Parfait. Interface d'état du bien en cours d'affichage.",
          niveauEnergetique: "Noté. Sélecteur DPE en cours d'affichage.",
          imagesbien: "Parfait. Interface photos en cours d'affichage.",
        };
        reply = FALLBACK[nextPopup] || reply;
      }
    }
    ...
    if (session.phase === "collecting") {
      ...
      // ── Triggers pop-ups vendeur ─────────────────────────────────────────
      // On renvoie le trigger + le reply qui CONTIENT déjà l'annonce (step2)
      if (missingProximite)
      ...
      if (stillMissing) return res.json({ reply, criteria: sc });
      ...
      // ── MATCHING — tous critères remplis ─────────────────────────────────
      const normalized = buildNormalized(sc, userRole);
      console.log("🔧 [PRE-DB]", JSON.stringify(normalized, null, 2));
    ...
    if (session.phase === "results") {
      ...
      return res.json({ reply, postReply: postAI.message || "", criteria: sc });
    }
